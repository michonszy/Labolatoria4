// Labolatoria4.cpp : Ten plik zawiera funkcję „main”. W nim rozpoczyna się i kończy wykonywanie programu.
//
...
public:
    ...
    int wiek(int rok_urodzenia) {
        int biezacy_rok = 2020;
        int wiek;
...
public:
    ...
    friend ostream& operator<<(ostream& out, const Emeryt& e) {
        out << "Osoba " << e.imie << " " << e.nazwisko << ", urodzony " << e.rok_urodzenia << " roku. " << "Emerytura: " << e.emerytura;
        return out;
    }
...
int main()
{
    ...
    //A
    Osoba osoba1("Janusz", "Osoba", 1999);
    int wiek = osoba1.wiek(1999);
    cout << "Twoój wiek to: " << wiek << endl;
    //B
    Informatyk* informatyk1 = new Informatyk("Szymon", "Informatyk", 1999, "slaskie", "Administrator Systemu", 4800, 2);

    //C
    Informatyk informatyk2(*informatyk1, "Jan", "Kopiujący", 1945);
    int wiek2 = informatyk2.wiek(1945);
    cout << "Twoój wiek to: " << wiek2 << endl;
    //D
    rozmiar = 3;
    Osoba** osoby = new Osoba * [rozmiar];
    osoby[0] = &osoba1; osoby[1] = informatyk1; osoby[2] = &informatyk2;
    int arraySize = sizeof(osoby);
    cout << "-----------WYŚWIETLENIE PĘTLI OSÓB-----------------------" << endl;
    
    for (int i = 0; i < rozmiar; i++) {
        
        cout << osoby[i]->getImie() << osoby[i]->getNazwisko() << osoby[i]->getRokUrodzenia() << endl;
        
    }
    cout << "Wielkosc tablicy osoby: " << rozmiar << endl;
    cout << "-----------------------EMERYCI----------------------------" << endl;

    
    generatorEmerytow(osoby);
  
    cout << "Wielkosc tablicy osoby: " << rozmiar << endl;

    cout << "-----------PĘTLA OSÓB BEZ EMERYTÓW-----------------------" << endl;
...
// Uruchomienie programu: Ctrl + F5 lub menu Debugowab nie > Uruchom bez debugowania
